...
class TradeController {
    ...
    def mostrarFormularioIntercambio() {
        ...
        // Obtener solo los amigos del usuario actual
        def amigos = FriendRequest.createCriteria().list {
            or {
                and {
                    eq("sender", usuarioActual)
                    eq("accepted", true)
                }
                and {
                    eq("receiver", usuarioActual)
                    eq("accepted", true)
                }
            }
        }.collect { it.sender == usuarioActual ? it.receiver : it.sender }

        def sets = Set.list()
        def cartasUsuario = Card.findAllByOwnerAndQuantityGreaterThan(usuarioActual, 0)

        render(view: 'formularioIntercambio', model: [
                usuarios     : amigos, // Solo amigos
                sets         : sets,
