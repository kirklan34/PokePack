...
class MainController {
    ...
    def abrirSobre(String setId) {
        ...
        session.packOpened = true // Marca el sobre como abierto
    ...
    def sobreAbierto() {
        if (!session.packOpened) {
            redirect(action: "abrirSobres") // Redirige si no se abrió un sobre
            return
        }

        session.packOpened = false // Resetea la bandera para evitar redirecciones futuras
        render(view: "sobreAbierto")
    }
    ...
    def abrirOtroSobre(String setId) {
        if (!setId) {
            render([success: false, message: "No se ha proporcionado un set válido."] as JSON)
            return
        }
        ...
        def set = Set.findBySetId(setId)
        if (!set) {
            render([success: false, message: "El set seleccionado no existe."] as JSON)
            return
        }

        // Lógica para abrir el sobre (similar a `abrirSobre`)
        def cartasDelSet = AllCards.findAllBySetName(set.name)
        if (!cartasDelSet || cartasDelSet.isEmpty()) {
            render([success: false, message: "No hay cartas disponibles para este set."] as JSON)
            return
        }

        // Selección de cartas (simplificada)
        Collections.shuffle(cartasDelSet)
        def cartasSeleccionadas = cartasDelSet.take(4) // Ejemplo: 4 cartas por sobre

        def resultado = cartasSeleccionadas.collect { card ->
            [
                    cardId: card.cardId,
                    name: card.name,
        ...
        render([success: true, cards: resultado] as JSON)
    }
